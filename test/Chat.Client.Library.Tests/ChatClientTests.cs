...
namespace Chat.Client.Library.Tests
{
    public class ChatClientTests : IDisposable
    {
        ...
        [Fact]
        public async Task LoginAsync_ShouldBeArgumentNullException_IfArgumentNullException()
        {
            //Arrange
            _userApi.Setup(user => user.LoginAsync("", ""))
                .Throws<ArgumentNullException>();

            //Act
            Func<Task> act = async () => { _ = await _chatClient.LoginAsync("", ""); };

            //Assert
            await act.Should().ThrowExactlyAsync<ArgumentNullException>().WithMessage("Value cannot be null.");
        ...
        [Fact]
        public async Task CreateUserAsync_ShouldBeArgumentNullException_IfArgumentNullException()
        {
            //Arrange
            _userApi.Setup(user => user.CreateUserAsync("", ""))
                .Throws<ArgumentNullException>();

            //Act
            Func<Task> act = async () => { _ = await _chatClient.CreateUserAsync("", ""); };
